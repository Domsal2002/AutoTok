...
from src.image_text import add_text_to_image, delete_image
...
def edit_video(background_path, title, body, config):
    ...
    # Add text to image and create image clip
    image_path = add_text_to_image(title)
    image_clip = ImageClip(image_path).set_duration(get_audio_duration(title_audio_file_path)).resize(height=video.h).set_position('center')
    ...
    final_video = CompositeVideoClip([video, image_clip] + subtitle_clips).set_audio(audio)
    final_video_path = 'final_video.mp4'
    final_video.write_videofile(final_video_path, codec='libx264', audio_codec='aac')
    
    # Clean up temporary files
    delete_image(image_path)
    os.remove(title_audio_file_path)
    os.remove(body_audio_file_path)
